				...
				<div className='cartDetails text-white'>
					<div className='container flex justify-center items-center flex-col pt-10'>
						<div className='flex gap-14'>
							...
							<div className={`button questBtn ${selectedTab === 'Brainquest' ? 'questBtnActive' : ''}`}
					...
					<div className='cartDetails-section container mt-8 flex gap-5 w-full items-start'>
						<div className='w-[39rem]'>
							<div className='mainCard p-2'>
								...
								<img src='https://res.cloudinary.com/drfni1iqf/image/upload/v1717399015/Tausif/temp/img_rjd4en.png' alt='' className='h-[410px] object-cover m-auto' />
							</div>
						...
						<div className='flex gap-2 flex-wrap justify-center w-full mb-10'>
							{noItem && (<div className='text-5xl mt-10 text-red-500'>Data are not available</div>)}
							{isLoading ? (<Loader />) : selectedTab === 'Wordexplore' ? (
								...
								cartDetails?.Storyadvenure.content.map((data, index) => (
									<div className='Storyadvenure w-full p-2 mb-4' key={index}>
										<img src={`https://ik.imagekit.io/dev24/${data?.Storyimage[ 0 ]}`} alt='' className='size-56 mr-4 mb-4 float-left' />
								...
								cartDetails?.Brainquest.map((data, index) => (
									<div className='cartOption w-[48%] p-2 mb-2' key={index}>
										...
										{data?.Option.map((option, index) => (
											<p className='text-xs my-2' key={option}><span className='font-bold'>opt-{index + 1}) </span> {option}</p>
										))}
										<p className='h-12'>
											<span className='font-bold ans' onClick={() => toggleShowAnswer(index)}>
												{showAnswer[ index ] ? 'Hide Answer: ' : 'Show Answer: '}
											</span>
											{showAnswer[ index ] && <span>{data?.Answer}</span>}
										</p>
									</div>
								))
							) : null}
